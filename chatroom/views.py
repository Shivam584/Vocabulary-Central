...
class chatroomPostChat(APIView):
    ...
    def post(self,request):
        ...
        if serializerd.is_valid():
            ...
            # validated_data['content']=transliterate_text(validated_data['content'],lang_code='en')
            # print(validated_data['content'])
            # print(validated_data['content'])
            # serializerd.save(content=validated_data['content'])
...
class chatroomJoinGroup(APIView):
    ...
    def post(self,request):
        ...
        return Response(serializerd.errors,status=status.HTTP_406_NOT_ACCEPTABLE)
